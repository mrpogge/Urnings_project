...
urnings_array = np.zeros((n_player,  2 * n_sim + 1, len(true_values) * len(player_urn_sizes)))
urnings_array_au = np.zeros((n_player, 2 * n_sim + 1, len(true_values * len(player_urn_sizes))))
...
for tv in range(len(true_values)):
    for pus in range(len(player_urn_sizes)):
        ...
        ###################################### Adaptive Urnsize algo ###################################
        #creating players and items
        players = []
        for i in range(n_player):
            pname = "player" + str(i)
            player = mu.Player(user_id = pname, score = 2, urn_size = 4, true_value = true_values[tv])
            players.append(player)
        ...
        game_rule = mu.Game_Type(adaptivity="adaptive", alg_type="Urnings2", paired_update= True, adaptive_urn= True, min_urn=min_urn, max_urn=max_urn, freq_change=8, window=window, bound=bound)
        game_sim = mu.Urnings(players = players, items = items, game_type = game_rule)
        game_sim.play(n_games=n_sim, test = True)
        for pl in players:
            pl.true_score = 0.5
        ...
        for pl in range(n_player):
            row = players[pl].container
            urnings_array_au[pl,:, counter] = row
        
        counter += 1

np.save("n_adaptive_us", urnings_array)
np.save("adaptive_us", urnings_array_au)